...
while any(open_mask)
    ...
    if isGoalReached(current.pose, goal_state, search)
        path = getFinalPath(nodes, current_index, stats, tic_id);
        return;
    end
...
function path = getFinalPath(nodes, goal_index, stats, tic_id)
...
yaw_idx = round((wrapToPiLocal(pose(3)) + pi) / search.yaw_grid_resolution_rad);
